---
name: Bug report
about: File a bug report to help us improve
title: ''
labels: bug
assignees: jonathandung

---

**Describe the bug**
A clear and concise description of what the bug is. Include the pattern implementation/utility function at fault, and the relevant snippet of the source if possible.

**To Reproduce**

```python
# minimal reproducible example here
```

**Expected behavior**
A clear and concise description of what you expected to happen.

**Screenshots**
If applicable, add screenshots to help explain your problem.

**Please complete the following information:**
 - Python version tag (`python -VV`)
 - asyncutils version (`python -m asyncutils -v` or `asyncutils -v`)
 - Operating system
 - Package version (`pip show py-asyncutils` or `conda list py-asyncutils`)
 - Full error traceback

**Additional context**
Add any other context about the problem here.
