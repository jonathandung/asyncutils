...
.. :data:: submodules_map:
  ...
  :note:
    For submodules that are not yet loaded, the value is an instance of an internal class with the same behaviour but not inserted into :data:
    `sys.modules`. This class is not a subclass of :class:`types.ModuleType` and provides a :meth:`load` method that fetches the submodule and
    replaces the entry in both :data:`submodules_map` and :data:`sys.modules`, and returns the real submodule object. For attribute accesses,
    it acts as a proxy to the real submodule, loading it when strictly required. The exact deference mechanism is an implementation detail.
