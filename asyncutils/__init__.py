...
if I.name != 'cpython': raise ImportError('asyncutils currently only supports cpython')
if I.version < (3, 15): raise ImportError('this version of asyncutils only supports python 3.15 or above')
from .version import VersionInfo as V
__hexversion__, preloaded_submodules = int(__version__ := V('0.8.18')), frozenset(('config', 'exceptions', 'version'))
