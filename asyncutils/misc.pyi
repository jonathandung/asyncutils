...
class CallbackAccumulator[T, **P](deque[Callable[P, T]], ExecutorRequiredAsyncContextMixin[CallbackAccumulator[T, P]]):
    ...
    @property
    def callbacks(self) -> Self: '''A view of the callbacks currently stored in the accumulator.'''
    def __iter__(self) -> GeneratorType[Callable[P, T]]: '''Iterate through the callbacks.'''
class CacheWithBackgroundRefresh[T, R](LoopMixinBase):
    '''
    ...
    async def __aenter__(self) -> Self: '''Start the background refresh loop.'''
    @overload
    async def __aexit__(self, exc_typ: ExcType, exc_val: BaseException, exc_tb: TracebackType, /) -> None: ...
    @overload
    async def __aexit__(self, exc_typ: None, exc_val: None, exc_tb: None, /) -> None: '''Stop the background refresh loop.'''
    ...
    async def refresh_loop(self) -> NoReturn: '''Check for entries requiring refresh and spawning tasks to do so in the background.'''
