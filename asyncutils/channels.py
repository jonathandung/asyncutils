...
class Observable(A.LoopContextMixin):
    ...
    def __init__(self, init_observers=(), maxsize=0): audit('asyncutils.channels.Observable', maxsize); self._data, self._lock, self._to_remove, self._queue, self._event = set(init_observers), I.Lock(), set(), None if maxsize is None else Queue(maxsize), I.Event()
    def __iter__(self): return self._data.__iter__()
    def __aiter__(self): return A.iter_to_agen(self._data)
    ...
    def subscribe_syncf(self, observer): return self.subscribe_nowait(A.to_async(observer, self.loop))
    ...
    def buffer(self, count, ret_exc=False):
        ...
        async def buffered(*a, **k):
            b.append((a, k))
            if len(b) >= c: await I.gather(*starmap(f, copy_and_clear(b)), return_exceptions=ret_exc)
...
class EventBus(A.LoopContextMixin):
    ...
    def event_names(self): (s := set(self._subscribers)).discard(None); return s
    ...
    async def event_stream(self, event_type=None, *, timeout=_NO_DEFAULT, item_timeout=_NO_DEFAULT, bufsize=None):
        ...
        t = await self.subscribe_until(F := self.loop.create_future(), partial(self.feed_event, timeout=A.getcontext().EVENT_BUS_STREAM_DEFAULT_TIMEOUT if timeout is _NO_DEFAULT else timeout), event_type); self.stream_queue = q = Queue(A.getcontext().EVENT_BUS_STREAM_DEFAULT_BUFFER_SIZE if bufsize is None else bufsize)
        if _NO_DEFAULT.is_(item_timeout): item_timeout = A.getcontext().EVENT_BUS_STREAM_DEFAULT_ITEM_TIMEOUT
        try:
    ...
    async def _safe_callback(self, c, d, t=None, i=None):
        try:
            async with self._sem:
                if I.iscoroutine(r := c(*filter_out(t, s=_NO_DEFAULT), d)): await I.wait_for(r, i)
...
class Rendezvous:
    ...
    def __init__(self, *, loop=None, lock=None): self._getters, self._putters, self._loop, self._lock = deque(), deque(), get_loop_and_set() if loop is None else loop, I.Lock() if lock is None else lock; self._make_task()
