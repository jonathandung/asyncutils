# ruff: file-ignore[compare-to-empty-string, print]
import sys as S
__all__ = 'bug', 'run'
def h(p, y=S.stdin.buffer.read): print(end=p); return y()
def bug(args): # ruff: ignore[complex-structure, too-many-branches, too-many-statements]
    # ruff: disable[read-whole-file]
    import asyncutils as A, importlib.metadata as m, urllib.parse as p; _, e, i, t, u, l, d, g, z, o, w = __import__('os').environ.get, args.ensure_filled, args.interactive, args.title, args.src_url, args.verbose-args.quiet, args.pastebin, args.log_path, args.traceback_path, args.open, S.stderr.write
    ...
    if i:
        ...
        if g == '': f, g = False, h('Paste the debug logs (not the file path): ' if l == 1 else 'Logs (debug level): ' if l == 0 else 'logs=')
        if z == '':
            f, z = False, h('Paste the traceback (not the file path): ' if l == 1 else 'Traceback: ' if l == 0 else 'tb=')
            if z:
    ...
    S.audit('asyncutils.cli.bug', s := f'https://github.com/jonathandung/asyncutils/issues/new?{p.urlencode({'template': 'bug.yaml', 'title': t, 'auv': A.__version__.representation, 'pkv': v, 'pyv': f'Python {S.version} on {S.platform}', 'os': __import__('platform').platform(), 'link': u, 'logs': post(g) if d else g, 'tb': post(z) if d else z, 'cfg': A._internal.unparsed.z, 'env': '' if args.no_prefill_env else '\n'.join(f'{k}={_(k, '')}' for k in ('AUTILSCFGPATH', 'AUTILSTESTMAXFAIL', 'FORCE_COLOR', 'NO_COLOR', 'PYTHON_BASIC_REPL', 'PYTHONSTARTUP', 'TERM'))}, quote_via=p.quote)}') # cspell: disable-line
    ...
    if args.print_on_fail:
        ...
        print(s)
    return 1
    # ruff: enable[read-whole-file]
def post(d):
    # ruff: disable[magic-value-comparison]
    if len(d) < 0x1000: return d
    if (c := (r := __import__('requests').post('https://paste.rs', d, timeout=(3.05, 27))).status_code) == 201: return r.text
    raise RuntimeError('asyncutils.cli.post: content truncated; erroring since logs or traceback should not be this big' if c == 206 else f'asyncutils.cli.post: could not upload to paste.rs due to {r.reason}; status: {c}')
...
def run(argv=None):
    ...
    try: return A.AsyncUtilsConsole().run(suppress_asyncio_warnings=p, suppress_unawaited_coroutine_warnings=p, always_run_interactive=len(a := S.orig_argv) == 2 and a[0] == S.executable and a[1].endswith(('/bin/autils.exe', '/bin/asyncutils.exe', r'\Scripts\asyncutils.exe', r'\Scripts\autils.exe'))) # ruff: ignore[magic-value-comparison]
