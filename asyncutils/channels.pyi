...
class Observable[**P](LoopContextMixin):
    ...
    @overload
    async def notify(self, *a: Any, _ret_exc_: bool=..., **k: Any) -> None: '''Notify the observers with the parameters passed in. If `_ret_exc_` is `True` (default `False`), exceptions occurring in any observer is not propagated.'''
    @overload
    def notify_sequential(self, *a: P.args, **k: P.kwargs) -> AsyncGenerator[Any]: ...
    @overload
    def notify_sequential(self, *a: Any, _silent_: bool=..., _persistent_: bool=..., **k: Any) -> AsyncGenerator[Any]: '''Version of :meth:`notify` that doesn't attempt to trigger the observers in parallel.'''
