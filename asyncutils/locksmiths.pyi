...
class LocksmithBase:
    ...
    async def force(self, lock: AsyncLockLike[Any], /, info: object=..., *, purge_waiters: bool=...) -> ForceResult:
        ...
        | `info`, if passed, should be an object representing the context of the force attempt, and will be passed to the exception thrown to the task
        | asking to release the lock forcefully.'''
    ...
    async def lock_busy(self, lock: AsyncLockLike[Any], requester: LocksmithBase, /) -> None: '''Do not call the :meth:`lock_busy` method of the requester in this method, since that will lead to infinite recursion.'''
    async def purge_waiters(self, lock: AsyncLockLike[Any], /) -> None: '''Clear all waiters on the lock after a force attempt, if necessary. The default implementation assumes this data is attached to the lock as its :attr:`_waiters` attribute, which is a data structure that evaluates to whether it still has any items in a boolean context, with a :meth:`pop` method.'''
