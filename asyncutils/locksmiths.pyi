...
class LocksmithBase:
    ...
    def __init__(self, loop: AbstractEventLoop|None=..., ltyp: type[AsyncLockLike[Any]]=...): '''Initialize the locksmith. ``loop`` is the event loop to use, defaulting to the current running loop. ``ltyp`` is the type of locks that this locksmith will attempt to force, defaulting to :class:`asyncio.Lock`.'''
    @property
    def currently_recognized(self) -> frozenset[AsyncLockLike[Any]]: '''Return a frozenset of locks that this locksmith currently recognizes.'''
    @classmethod
    def register_handler[T: AsyncLockLike[Any]](cls, h: Callable[[T], object], /, *, shadow: bool=...) -> Callable[[type[T]], type[T]]: '''A decorator factory for async lock classes, taking a handler function and returning an identity decorator.'''
    ...
    async def host[T](self, task: Awaitable[T], lock: AsyncLockLike[Any], /, *, timeout1: float|None=..., timeout2: float|None=..., timeout3: float|None=...) -> T: '''Run ``task`` holding ``lock`` immediately after forcing it. The default values of the timeouts are taken from :const:`context.LOCKSMITH_BASE_DEFAULT_TIMEOUTS`.'''
    async def get_info(self, lock: AsyncLockLike[Any], /) -> Any: '''Return information about the lock that will be passed to the forcing request.'''
    async def lock_busy(self, lock: AsyncLockLike[Any], requester: LocksmithBase, context: dict[str, Any], /) -> None: '''Called when a :exc:`~exceptions.LockForceRequest` by a different locksmith propagates, meaning that another locksmith is trying to do the same thing. The ``context`` parameter is a dictionary that can be passed to ``extra`` of a log record, for example. The implementation is allowed to call :meth:`lock_busy` on the requester in this method with a modified ``context``, but care should be taken to avoid infinite recursion.'''
    async def purge_waiters(self, lock: AsyncLockLike[Any], /) -> None: '''Clear all waiters on the lock after a force attempt, if necessary. The default implementation assumes this data is attached to the lock as its ``_waiters`` attribute, which is a data structure that evaluates to whether it still has any items in a boolean context, with a :meth:`~collections.abc.MutableSequence.pop` method.'''
    ...
    def task_raised_critical(self, lock: AsyncLockLike[Any], exc: BaseException, /) -> Literal[ForceResult.FAILURE]: '''Called when a task raises a critical exception in response to a force request. The default implementation throws the exception wrapped in :class:`exceptions.Critical`, rather than returning any value to :meth:`force`. This is not async because it is imperative that the exception be dealt with quickly. Subclasses can choose to return :const:`ForceResult.FAILURE` after some handling as well.'''
