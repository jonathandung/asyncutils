...
class D(W):
    def __init__(self, d, /): super().__init__(); self.cs, self.dv = bool, d
...
@subscriptable
class AsyncPropertyBase(A.LoopBoundMixin, metaclass=type('AsyncPropertyMeta', (abc.ABCMeta,), {'__prepare__': classmethod(lambda c, n, b, _=D, /, **k: (p := c.__base__.__prepare__(n, b, **k)).update(__doc__=_(None), __module__=_('asyncutils.properties'), __name__=_(n)) or p)})):
    ...
    def __get__(self, o, _=None, /):
        ...
        self._check_unbound()
        if self.fget is None: return self._get_helper('asyncutils.properties.AsyncPropertyBase: unreadable attribute')
        if o in self.__deleted: return self._get_helper('asyncutils.properties.AsyncPropertyBase: cannot get deleted attribute')
        return self._get(o)
    def __set__(self, o, v, /):
        if o is None: return self._check_hidden()
        self._check_unbound()
        if (f := self.fset) is None: return self._set_helper('asyncutils.properties.AsyncPropertyBase: immutable attribute', v)
        if o in self.__deleted: return self._set_helper('asyncutils.properties.AsyncPropertyBase: cannot set deleted attribute', v)
    ...
    def __repr__(self): return f'asyncutils.properties.{type(self).__name__}({self.fget!r}, {self.fset!r}, {self.fdel!r}, doc={self.__doc__!r}, strict={self.__strict}, hide={self.__hide})'
    def __reduce__(self):
    ...
    def _get_helper(self, m, /):
        ...
        return self
    @abc.abstractmethod
    def _inner_helper(self, _, /): raise NotImplementedError
...
class LazyAsyncProperty(AsyncPropertyBase, lock_factory=__import__('asyncio').Lock): __slots__, _inner_helper = (), staticmethod(A.wrap_in_coro)
class ConcurrentAsyncProperty(AsyncPropertyBase, lock_factory=staticmethod(lambda _=A.anullcontext, /: _)): __slots__, _inner_helper = (), A.LoopBoundMixin.make
