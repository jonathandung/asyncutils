import sys as S, asyncutils._internal.patch as P
...
if (f := getattr(S, '_getframemodulename', None)) is None: # pragma: no cover
    def f(depth=0, _=S._getframe): return _(depth).f_globals.get('__name__')
class SentinelBase:
    _can_instantiate = False; __slots__ = '__m', '__n'
    ...
    def __repr__(self): return f'<{fullname(self)} {self.__n!r} at {id(self):#x}>'
    def __str__(self): return getattr(self, '_SentinelBase__n', '<unbound>')+(' <private>'*self.is_private)
    ...
    def __reduce__(self):
        if (n := getattr(self, '_SentinelBase__n', None)) is None: raise TypeError(f'cannot pickle unbound instance of {fullname(self)}')
        return type(self), (n,)
    ...
    @property
    def bound_to(self): return getattr(self, '_SentinelBase__n', '').rpartition('.')[0] or None
    @property
    def back(self): return getattr(self, '_SentinelBase__n', '').rpartition('.')[2] or None
...
_NO_DEFAULT, NO_COALESCE, RAISE = map(_Sentinel, ('_NO_DEFAULT', 'NO_COALESCE', 'RAISE'))
