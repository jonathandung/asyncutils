...
class TokenBucket:
    ...
    @property
    def capacity(self) -> JustFloat: '''The capacity of the bucket.'''
class LeakyBucket(AsyncContextMixin[LeakyBucket], LoopMixinBase):
    ...
    | The bucket includes an adaptive factor that adjusts based on current fill level to provide smoother rate limiting under varying loads, as dictated by :const:`~asyncutils.context.Context.LEAKY_BUCKET_ADJMAP`, a sequence of tuples ``(min_capacity, (lbound, lfactor, ubound, ufactor))`` monotonically descending in ``min_capacity``.
    '''
    def __init__(self, capacity: float, leak: float, min_factor: float=..., max_factor: float=..., external_factor_settable: bool=..., timer: Timer=...):
        ...
        * ``min_factor`` (optional): Minimum adaptive factor; default :const:`~asyncutils.context.Context.LEAKY_BUCKET_DEFAULT_MIN_FACTOR`.
        * ``max_factor`` (optional): Maximum adaptive factor; default :const:`~asyncutils.context.Context.LEAKY_BUCKET_DEFAULT_MAX_FACTOR`.
        * ``external_factor_settable`` (optional): Whether the factor attribute can be modified; default :const:`~asyncutils.context.Context.LEAKY_BUCKET_DEFAULT_EXT_CAN_SET_FACTOR`.
        '''
    @overload
