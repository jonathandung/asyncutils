...
class LeakyBucket(AsyncContextMixin[LeakyBucket], LoopMixinBase):
    ...
    | :const:`~asyncutils.context.Context.LEAKY_BUCKET_ADJMAP`, a sequence of tuples ``(mincap, (lbound, lfactor, ubound, ufactor))`` monotonically descending in ``mincap``.'''
    ...
    async def wait_for_tokens(self, amount: float=...) -> float:
        '''| Keep asynchronously sleeping, with a maximum interval of :const:`~asyncutils.context.Context.LEAKY_BUCKET_WAIT_FOR_TOKENS_TICK` each time, until ``amount``
        | tokens can be added to the bucket *at once* (default :const:`~asyncutils.context.Context.LEAKY_BUCKET_DEFAULT_WAIT_FOR_TOKENS_TOKENS`), and do so, returning
        | the total wait time.
