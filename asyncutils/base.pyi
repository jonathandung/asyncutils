...
class event_loop: # noqa: N801
    ...
    def factory_reset(self) -> None: '''Restore the default settings from the context (i.e., set the flags to :data:`context.EVENT_LOOP_BASE_FLAGS`).'''
    ...
    @classmethod
    def from_flags(cls, flags: int, /) -> Self: '''Construct an instance from ``flags``, a bitwise or of options (default :data:`context.EVENT_LOOP_BASE_FLAGS`).'''
...
def iter_to_agen[T](it: Iterable[T], sentinel: T, *, use_existing_executor: bool=..., create_executor: bool=..., strict: bool=...) -> AsyncGenerator[T]:
    ...
    | When ``use_existing_executor=True`` is passed (default :data:`context.ITER_TO_AGEN_DEFAULT_USE_EXISTING_EXECUTOR`), the function will attempt to use
    | an existing executor as created by previous calls specifying ``create_executor=True`` (default :data:`context.ITER_TO_AGEN_DEFAULT_MAY_CREATE_EXECUTOR`)
    | to advance the iterable, and fall back to blocking the event loop every step without an executor.
    | If ``strict`` is ``True`` (default :data:`context.ITER_TO_AGEN_DEFAULT_STRICT`), only sync iterables are accepted.'''
@overload
...
def aiter_to_gen[T](ait: Iterable[T], *, use_futures: bool=..., loop: AbstractEventLoop|None=..., strict: Literal[False]=...) -> Generator[T]:
    ...
    | If the event loop is currently running and ``use_futures`` is ``False`` (default :data:`context.AITER_TO_GEN_DEFAULT_ALLOW_FUTURES`), raise :exc:`RuntimeError` to clarify that :class:`concurrent.futures.Future` must be used in this case, one per item yielded,
    | which is somewhat inefficient, but that can't be helped.
    | If ``strict`` is ``True`` (default :data:`context.AITER_TO_GEN_DEFAULT_STRICT`), only async iterables are accepted.'''
...
async def collect[T](it: SupportsIteration[T], n: int|None=..., *, default: T|RaiseType=...) -> list[T]:
    ...
    | If there are less than ``n`` items to collect, throw :exc:`exceptions.ItemsExhausted` if default is :data:`constants.RAISE` and emit a debug
...
def take[T](it: SupportsIteration[T], n: int|None) -> AsyncGenerator[T]:
    ...
    .. tip::
      ...
      In particular, pass :data:`constants.RAISE` as ``default`` to cause :exc:`exceptions.ItemsExhausted` to be thrown in the case that there
      aren't enough items.
