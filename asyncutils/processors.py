...
class BoundedBatchProcessor:
    ...
    def __init__(self, processor, batch=None, max_concurrent=None): C = A.getcontext(); self._processor, self._batch, self._sem = processor, C.BOUNDED_BATCH_PROCESSOR_DEFAULT_BATCH_SIZE if batch is None else batch, Semaphore(C.BOUNDED_BATCH_PROCESSOR_DEFAULT_MAX_CONCURRENT if max_concurrent is None else max_concurrent)
...
class BatchProcessor(A.LoopContextMixin):
    ...
    def __init__(self, processor, *, maxsize=None, maxtime=None, timer=monotonic): C = A.getcontext(); self._processor, self._maxsize, self._sleep, self._batch, self._last_process, self._lock, self._timer = processor, C.BATCH_PROCESSOR_DEFAULT_MAX_SIZE if maxsize is None else maxsize, sleep.__get__(C.BATCH_PROCESSOR_DEFAULT_MAX_TIME if maxtime is None else maxtime), [], timer(), Lock(), timer; self._flusher = None
...
class Bulkhead(A.LoopContextMixin):
    ...
    async def execute(self, coro):
        ...
        async with self._sem:
            ...
            except (QueueEmpty, QueueShutDown, CancelledError): raise A.BulkheadShutDown(f'{fullname(self)} is shutting down') from None
