...
class Bulkhead(A.LoopContextMixin):
    ...
    def __init__(self, max_concurrent, *, max_queue=None, max_rej=None, exc=Exception, processor=None):
        ...
        super().__init__(); self._sem, self._queue, self._rejected, self._init_val, self._exc, self._processor, self._sdfut, self._mtevt, self._max_rej = I.Semaphore(max_concurrent), Queue(max_queue), 0, max_concurrent, exc, processor, self.make_fut(), I.Event(), max_rej
    ...
    async def wait_until_idle(self, timeout=None): await I.wait_for(self._mtevt.wait(), timeout)
    def wait_for_shutdown(self, timeout=None): return I.wait_for(self._sdfut, timeout)
