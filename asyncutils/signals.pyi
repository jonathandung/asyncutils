...
async def wait_for_signal[T](processor: Callable[[Signals], T], /, *S: int, timeout: float|None=..., raise_on_timeout: Literal[False]=..., loop: AbstractEventLoop|None=..., possible_errors: tuple[ExcType, ...]=..., default_on_processor_failure: T, sigs: Iterable[int]=..., logger: Logger=...) -> T|None:
    ...
    | If `loop` is passed, its :meth:`add_signal_handler` and :meth:`remove_signal_handler` methods will be used.
    | Errors whose types are included in `possible_errors` will cause `logger` to emit an error and the function to return `default_on_processor_failure` (or `None` if not passed).
    ...
    | Note that there is limited support for signals on Windows and this function may not work as expected even with the little signals it provides.'''