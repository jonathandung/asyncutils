...
class BulkheadFull(BulkheadError): '''Raised when a bulkhead is full and a party attempts to execute a coroutine under it.'''
class BulkheadShutDown(BulkheadError): '''Raised when a bulkhead is being shut down and a party attempts to execute a coroutine under it.'''
...
class ResourceBusy(RuntimeError): '''Raised when a party attempts to use a resource guarded actively by a :class:`~asyncutils.altlocks.ResourceGuard`.'''
...
class MoreThanOne[T](ValueError):
    '''Raised when :func:`~asyncutils.iters.at_most_one` finds more than one item in the iterable it was passed.'''
    def __init__(self, it: AsyncGeneratorType[T], msg: str, /): ...
    @property
...
class LockForceRequest[T, S: AsyncLockLike[Any], R: LocksmithBase](BaseException):
    ...
    def __init__(self, requester: R, fulfil: Callable[[Any], None], lock: S, info: T, /): ...
    @property
    ...
    def fulfil(self, answer: object, /) -> None: '''Answer the request with ``answer``, after presumably releasing the lock and performing error handling.'''
...
class ForbiddenOperation(PasswordQueueError, TypeError):
    ...
    def __init__(self, op: str, *a: object): '''Substitute in the arguments to the string (if any) to derive the name of the forbidden operation.'''
class PasswordError[T](PasswordQueueError):
    ...
    @property
    def wrong_pwd(self) -> T: '''The wrong password associated with the exception. May be ``None`` if the wrong password has been garbage collected.'''
    @property
...
class WrongPasswordType[T, R: type](PasswordError[T], TypeError):
    ...
    @property
    def correct_type(self) -> R|None: '''The correct password type associated with the exception. May be ``None`` if the wrong password type has been garbage collected, indicating its instances, and thus the password, and by extension the queue itself, have been destroyed.'''
...
class IgnoreErrors:
    ...
    def __init__(self, /, *exc: ExcType, exclude: Iterable[ExcType]=...): '''Suppress the exception classes specified by the positional arguments within the context, and consume the iterable ``exclude`` to build up :attr:`but`. All overlap is discarded.'''
...
class WarningToError:
    ...
    def __init__(self, /, *types: type[Warning]): '''Positional arguments represent warning types to be raised as errors if they are to occur within the context.'''
...
ignore_stop_iteration: Final[IgnoreErrors]
