...
'''
    ...
    async def readall(self) -> T: '''Read all characters from the reader until EOF. If the :meth:`readall` method is not present, call :meth:`read` with no arguments without handling the non-blocking case.'''
...
async def ainput(prompt: str=..., assert_tty: bool=...) -> str: '''Asynchronously write `prompt` to standard output and read a line from standard input, returning it without the trailing newline. If `assert_tty` is `True`, raise :exc:`OSError` if standard input is not a TTY.'''
