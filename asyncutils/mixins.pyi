...
class LockMixin[T](ABC):
    ...
    @abstractmethod
    def locked(self) -> bool: '''Return whether the lock is currently held by a task satisfying a certain criterion (e.g. it is exactly the task that awaited the :meth:`acquire` call).'''
    def acknowledge_locksmith_lock_held(self, smith: LocksmithBase, /) -> bool|Coroutine[Any, Any, bool]: '''Cooperate with locksmiths in case of unusually long wait times for the lock which may indicate deadlock, livelock or starvation. Optional.'''
