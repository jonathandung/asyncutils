...
class event_loop: # ruff: ignore[invalid-class-name]
    ...
    def _get_unclosed_loop(self, factory=I.new_event_loop, _=A.IgnoreErrors(AttributeError)): # pragma: no cover
        ...
        if not self._flags&c.NEVER_CLEAR_TASKS_ON_REUSE:
            with _: L._ready.clear()
            with _: L._scheduled.clear()
        return L
    def factory_reset(self): self._flags = self.Flags(A.getcontext().EVENT_LOOP_BASE_FLAGS)
    def clear_flags(self, mask_to_keep=0): self._flags &= mask_to_keep
    def copy_flags(self): return self.from_flags(self._flags)
    def __contains__(self, f, /): return bool(self._flags&(self.Flags[f.upper()] if isinstance(f, str) else f))
    ...
    def __enter__(self, _='asyncutils.base.event_loop: context already entered'):
        ...
        else: s |= S.CREATED_LOOP
        if not f&c.NEVER_ENTER and callable(g := getattr(l, '__enter__', None)): # pragma: no cover
    ...
    def __exit__(self, t, v, b, /, _m='%s context not entered', _n='%s context not entered with errors passed into __exit__', _i=A.IgnoreErrors(RuntimeError), _l=L): # ruff: ignore[complex-structure,too-many-branches,too-many-statements]
        ...
        if not (s := self._state)&S.ENTERED:
            ...
            return False
        if f&d.CANCEL_ALL_TASKS: self._task = l.create_task(safe_cancel_batch(I.all_tasks(l)))
        if not f&d.DISALLOW_REUSE: self.__reusable.append(l)
        if not ((c := s&S.CREATED_LOOP) and f&d.SOMETIMES_CONTINUE_ON_EXIT):
            with _i: l.stop()
        q, r, self._state = t is not None and issubclass(t, RuntimeError), False, s-S.ENTERED
        if s&S.ENTERED_INNER: # pragma: no cover
            if callable(g := getattr(l, '__exit__', None)):
                try: r = g(None, None, None) if f&d.SUPPRESS_INNER_EXIT_ON_RUNTIME_ERROR and q else g(t, v, b)
                except A.CRITICAL: _l.critical('%s: critical error while calling __exit__ of associated event loop', n, exc_info=True)
                except RuntimeError:
                    if z: _l.exception('event loop management shenanigans while exiting associated event loop')
                except:
                    if a: _l.exception('%s: exception occurred while calling __exit__ of associated event loop', n)
            elif a: _l.error('%s: __enter__ already called but __exit__ is not present', n)
        if s&S.AENTERED_INNER: # pragma: no cover
            if callable(g := getattr(l, '__aexit__', None)) and not r:
                l.stop(); l.run_forever()
                try: r = l.run_until_complete(g(None, None, None) if f&d.SUPPRESS_INNER_AEXIT_ON_RUNTIME_ERROR and q else g(t, v, b)) or r
                except A.CRITICAL: _l.critical('%s: critical error while calling __aexit__ of associated event loop', n, exc_info=True)
                except RuntimeError:
                    if z: _l.exception('runtime error exiting associated event loop')
                except:
                    if a: _l.exception('%s: exception occurred while calling __aexit__ of associated event loop', n)
            elif a: _l.error('%s: __aenter__ already called but __aexit__ is not present', n)
        if f&d.CLOSE_EXISTING_ON_EXIT or not (c or f&d.KEEP_CREATED_OPEN_ON_EXIT):
            with _i: l.close()
    ...
    def __del__(self, _f=L.debug, _g=L.warning, _m='%s: garbage-collecting entered context; you are advised to refactor your code', _w='%s: cannot suppress exceptions from within destructor', _d='destroyed %s'): # pragma: no cover
        ...
        if not self._state&self.State.ENTERED:
            ...
            return
        if b: _g(_m, n)
        if f&c.FLIP_RELEASE_LOOP_ON_FINALIZATION: self._flags = f^c.DISALLOW_REUSE
    ...
    P.patch_method_signatures((__enter__, ''), (__exit__, P.exit_sig), (__del__, ''), (_get_unclosed_loop, 'factory={}')); P.patch_classmethod_signatures((from_flags, 'flags, /'), (__new__, f'*, {'={0}, '.join(Flags._member_names_)}={{0}}'))
def f(n):
    ...
    return adisembowel
adisembowel, adisembowel_left = map(f, ('pop', 'popleft'))
...
async def iter_to_agen(it, sentinel=_NO_DEFAULT, *, use_existing_executor=None, create_executor=None, strict=None, a=c, b=b, c=H.check, s=H.create_executor, h=H.get_loop_and_set, w=L.debug, d=0x400, _=type('', (), {'__slots__': ('it',), '__init__': lambda self, it: setattr(self, 'it', it), '__bool__': lambda self, _=b: _(self.it, 'send', 'throw', 'close'), '__enter__': lambda self: None, '__exit__': lambda self, t, v, _, /, e=frozenset((StopAsyncIteration, StopIteration)), f=frozenset(('StopIteration interacts badly with generators and cannot be raised into a Future', 'async generator raised StopIteration')): False if t is None else str(v) in f if t is RuntimeError else (((True if (C := getattr(self.it, 'close', None)) is None else C()) if t in e else (True if (T := getattr(self.it, 'throw', None)) is None else T(v))) or True)})): # ruff: ignore[complex-structure,too-many-arguments,too-many-branches,too-many-statements,unused-lambda-argument]
    # ruff: disable[yield-in-context-manager-in-async-generator]
    audit('asyncutils.base.iter_to_agen', a(it))
    if type(it) in Z.s:
        for i in batched(it, d):
            for _ in i: yield _
    ...
    with g:
        if e is None:
            ...
            def r(*a, _=h().run_in_executor, e=e): return partial(_, e, *a) # ruff: ignore[function-call-in-default-argument]
            if g:
...
def aiter_to_gen(ait, *, use_futures=None, loop=None, strict=None, a=c, b=b, g=H.get_loop_and_set):
    ...
    with A.ignore_stop_async_iteration:
...
async def take(it, n=None, default=_NO_DEFAULT, _=L.debug, m='asyncutils.base.take: ran out of items to take'):
    ...
    if n == 0: return
    async for n, i in aenumerate(it, n-1, -1): # ruff: ignore[loop-variable-overrides-iterator,redefined-argument-from-local]
        yield i
        if n == 0: return
...
(dummy_task := type(_)(_.__code__.replace(co_flags=353, co_name=(_ := 'dummy_task'), co_qualname=_), globals())(None)).close()
del f, _, P, L, b, c, H
