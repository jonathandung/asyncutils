...
async def first_completed[T](*C: Awaitable[T], ret_exc: Literal[False]=..., timeout: float|None=...) -> T|None:
    ...
    | If `ret_exc` is `True`, the coroutine might have errored, in which case the exception it throws is returned in a wrapped form unpackable
    | using :func:`exceptions.unwrap_exc` after checking with :func:`exceptions.exception_occurred`.
...
async def race_with_callback[T](*C: Awaitable[T], winner: Callable[[T], object]=..., loser: Callable[[Any|BaseException], object]=..., timeout: float|None=...) -> T|None:
    ...
    | The `loser` callback is called on each return value of or exception raised by the losing coroutines after seeing :exc:`~asyncio.exceptions.CancelledError`.'''
async def multi_winner_race_with_callback[T](*C: Awaitable[T], timeout: float, winner: Callable[[T], object]=..., loser: Callable[[Any|BaseException], object]=...) -> list[T]: '''Return a list of all the coroutines that completed within `timeout`, and cancel the rest, triggering callbacks similarly to race_with_callback.'''
def convert_to_coro_iter(cfs: SupportsIteration[Any], *, skip_invalid: bool=..., loop: AbstractEventLoop|None=..., corocheck: Callable[[Any], TypeGuard[Coroutine[Any, Any, Any]]]=..., futwrap: FutWrapType=..., handle_aiter: Callable[[AsyncIterable[Any]], Coroutine[Any, Any, Any]]=..., handle_iter: Callable[[Iterable[Any]], Coroutine[Any, Any, Any]]=...) -> Generator[Coroutine[Any, Any, Any], Any, None]:
    '''| A helper function to convert a possibly async iterable of futures `cfs`, coroutines and even (async) iterables to a plain generator of coroutines,
    ...
    | `skip_invalid`, which determines whether to raise :exc:`TypeError` for unconvertible items or simply to skip them, defaults to :const:`context.CONVERT_TO_CORO_ITER_DEFAULT_SKIP_INVALID`.
...
async def enhanced_gather(it: SupportsIteration[Any], return_exceptions: bool=False, *, loop: AbstractEventLoop|None=...) -> list[Any]:
    ''':func:`asyncio.gather`, but taking a larger variety of objects as the first argument using :func:`convert_to_coro_iter`; see above.
    ...
    .. seealso::
      ...
      :func:`iters.agather`
        ...
        futures, which in many cases is a better strategy
'''