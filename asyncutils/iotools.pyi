...
def double_ended_text_pipe(*, pipe_impl: Callable[[], tuple[int, int]]=...) -> tuple[AsyncReadWriteCouple[str, str], AsyncReadWriteCouple[str, str]]:
    ...
    '''
def double_ended_binary_pipe(*, pipe_impl: Callable[[], tuple[int, int]]=...) -> tuple[AsyncReadWriteCouple[bytes, bytes], AsyncReadWriteCouple[bytes, bytes]]: ''':func:`double_ended_text_pipe`, but opens the pipes in binary mode.'''
...
class MemoryMappedIOManager(LoopContextMixin):
    ...
    @property
    def open_paths(self) -> dict[Openable, Literal['r+b', 'w+b', 'x+b']]: '''Dictionary mapping file paths as passed to :meth:`open`, :meth:`create` or :meth:`create_sparse_file` to the mode the file was opened with.'''
    @property
    def open_files(self) -> OpenFiles: '''Dictionary mapping tuples of the form ``(file, mode)`` to the managed file objects.'''
    def open(self, path: Openable, init_size: int=...) -> OpenRV: '''Open a file at ``path`` for memory-mapped reading and writing on entry and close it on exit. The file must exist and is not truncated.'''
    def create(self, path: Openable, init_size: int=..., *, exclusive: bool=...) -> OpenRV: '''Open a file at ``path`` for memory-mapped writing and reading on entry and close it on exit. The file is truncated to the beginning, and must not exist if ``exclusive`` is ``True`` (the default behaviour).'''
    def create_sparse_file(self, path: Openable, total_size: int, chunks: Mapping[int, bytes|str]) -> OpenRV: '''Return an async context manager that creates a file of size ``total_size`` at ``path``, with ``chunks`` mapping offsets to the data to be written there. Data from old chunks is overwritten by that from new ones.'''
    ...
    async def copy_file(self, src: Openable, dest: Openable, *, exclusive: bool=..., flush: bool=...) -> None: '''Copy the contents of the file at ``src`` into that at ``dest`` asynchronously and flush it if ``flush`` is ``True``. Uses :meth:`open` and :meth:`create` internally.'''
