...
class UniqueResourceGuard(ResourceGuard):
    ...
    .. tip:: The object has to be hashable, and a strong reference will be held to it for the lifetime of the guard.
...
class CircuitBreaker:
    ...
    def __new__[T, **P](cls, f: Callable[P, Awaitable[T]], /, max_fails: int=..., reset: float|None=..., *, exc: Exceptable=..., max_half_open_calls: int|None=...) -> Callable[P, Coroutine[Any, Any, T]]: # type: ignore[misc]
        ...
        | When the decorated function fails more than `max_fails` times (default :const:`context.CIRCUIT_BREAKER_DEFAULT_MAX_FAILS`), the breaker triggers (opens the circuit) and
        | disallows further calls of the wrapped functions by throwing an exception.
        ...
        | If the function completes successfully when the breaker is half-open under `max_half_open_calls` (default :const:`context.CIRCUIT_BREAKER_DEFAULT_MAX_HALF_OPEN_CALLS`)
        | tries, the circuit closes automatically. Otherwise, the circuit reopens.'''
