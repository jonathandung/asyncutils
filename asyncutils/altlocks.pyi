...
class ResourceGuard[T](AsyncContextMixin[None]):
    ...
    @property
    def success_ratio(self) -> JustFloat: '''The current ratio of successful acquisitions to total acquisition attempts, or ``0.0`` if there have been no attempts.'''
    @overload
...
class Releasing:
    ...
    def __init__(self, lock: AsyncLockLike[object], /): '''Instantiate the async context manager to release ``lock`` on entry and re-acquires it on exit.'''
...
class DynamicThrottle:
    ...
    @property
    def jitter(self) -> JustFloat: '''The current jitter in calculating the wait time.'''
    @jitter.setter
    def jitter(self, jitter: float, /) -> None: '''Set the jitter to ``jitter``.'''
    @property
    def rate(self) -> JustFloat: '''The current rate.'''
    @rate.setter
