...
class CircuitBreaker:
    '''The circuit breaker pattern. Instances can be used as decorators, unless instantiated with a function as the first parameter, in which case the decorated function is returned
    and the name of the circuit breaker (for debugging) derived from the name of the function.
    Use on async functions that may fail unpredictably, passing exceptions that are expected to happen through the exc parameter..
    The circuit is initially closed. When the decorated function fails more than `max_fails` times, the breaker triggers and disallow further calls of the same function by throwing an exception.
    (open the circuit). This state persists until the reset timeout expires, the default of which can be changed in the constants submodule. Then, the breaker enters the half-open state.
...
class DynamicThrottle:
    '''An async context manager used to limit the rate of a function being called.'''
