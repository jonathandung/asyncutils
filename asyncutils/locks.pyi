...
| :class:`~asyncutils._internal.prots.AsyncLockLike` protocol, besides :class:`MultiCountDownLatch`, since it uses
...
class DynamicBoundedSemaphore(BoundedSemaphore):
    ...
    def __init__(self, value: int=...): '''`value`, the initial value of the semaphore, defaults to :data:`context.DYNAMIC_BOUNDED_SEMAPHORE_DEFAULT_VALUE`.'''
    @property
...
class AdvancedRateLimit(LoopMixinBase, LockMixin[None]):
    ...
    async def acquire(self, tokens: float=..., timeout: float|None=...) -> Literal[True]: '''Acquire the specified number of tokens from the rate limiter (default :data:`context.ADVANCED_RATE_LIMIT_DEFAULT_TOKENS`), waiting until the timeout expires and signalling :exc:`TimeoutError` if necessary.'''
    async def release(self, tokens: float=...) -> None: '''Release the specified number of tokens back to the rate limiter (default :data:`context.ADVANCED_RATE_LIMIT_DEFAULT_TOKENS`).'''
...
class PrioritySemaphore(LoopMixinBase, LockMixin[None]):
    ...
    def __init__(self, value: int=...): '''`value`, the initial value as an integer, defaults to :data:`context.PRIORITY_SEMAPHORE_DEFAULT_VALUE`.'''
    async def acquire(self, priority: int=...) -> Literal[True]: '''Acquire the semaphore with the specified priority (default :data:`context.PRIORITY_SEMAPHORE_DEFAULT_PRIORITY`).'''
