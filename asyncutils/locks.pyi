...
class LocksmithBase:
    ...
    async def host[T](self, task: Awaitable[T], lock: AsyncLockLike[Any], /, *, timeout1: float|None=..., timeout2: float|None=..., timeout3: float|None=...) -> T: '''Run `task` holding `lock` immediately after forcing it. The default values of the timeouts are taken from :const:`context.LOCKSMITH_DEFAULT_TIMEOUTS`.'''
    async def _wait_on[T](self, task: Awaitable[T], lock: AsyncLockLike[Any], /) -> T: ...
    async def get_info(self, lock: AsyncLockLike[Any], /) -> Any: ...
    async def lock_busy(self, lock: AsyncLockLike[Any], requester: LocksmithBase, /) -> None: '''Do not call the :meth:`lock_busy` method of the requester, since that will lead to infinite recursion.'''
