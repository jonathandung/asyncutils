...
class DynamicBoundedSemaphore(BoundedSemaphore):
    ...
    def __init__(self, value: int=...): '''``value``, the initial value of the semaphore, defaults to :const:`~asyncutils.context.Context.DYNAMIC_BOUNDED_SEMAPHORE_DEFAULT_VALUE`.'''
    @property
    def bound(self) -> int: '''The bound of the semaphore. When modified, wake up waiters if the internal value is greater than the new bound.'''
    @bound.setter
...
class AdvancedRateLimit(LoopMixinBase, LockMixin[None]):
    ...
    def __init__(self, rate: float, capacity: float=..., fair: bool=...):
        ...
        * ``fair``: Whether to maintain FIFO (first in, first out) for waiters; default ``True``.
        '''
    async def acquire(self, tokens: float=..., timeout: float|None=...) -> Literal[True]: '''Acquire the specified number of tokens from the rate limiter (default :const:`~asyncutils.context.Context.ADVANCED_RATE_LIMIT_DEFAULT_TOKENS`), waiting until the timeout expires and signalling :exc:`TimeoutError` if necessary.'''
    async def release(self, tokens: float=...) -> None: '''Release the specified number of tokens back to the rate limiter (default :const:`~asyncutils.context.Context.ADVANCED_RATE_LIMIT_DEFAULT_TOKENS`).'''
    ...
    @property
    def rate(self) -> float: '''The current rate of the limiter.'''
    @property
    def tokens(self) -> float: '''The current number of tokens available.'''
    @property
    def capacity(self) -> float: '''The capacity of the limiter.'''
class PrioritySemaphore(LoopMixinBase, LockMixin[None]):
    ...
    def __init__(self, value: int=...): '''``value``, the initial value as an integer, defaults to :const:`~asyncutils.context.Context.PRIORITY_SEMAPHORE_DEFAULT_VALUE`.'''
    async def acquire(self, priority: int=...) -> Literal[True]: '''Acquire the semaphore with the specified priority, defaulting to ``0``.'''
...
class KeyedCondition[T](LockMixin[KeyedCondition[T]], LoopContextMixin):
    ...
    def assert_locked(self) -> None: '''Ensure that the underlying lock is currently locked, raising a :exc:`RuntimeError` if not.'''
...
class MultiCountDownLatch[T: Hashable]:
    ...
    async def count_down(self, key: T, strict: bool=...) -> None: '''Decrement the count of the latch with the given key. If it reaches zero, wake up all waiters. If ``strict`` is ``True`` and the key doesn't exist, :exc:`KeyError` is raised.'''
    async def count_down_all(self) -> None: '''Decrement the count of each latch.'''
    async def wait(self, key: T, strict: bool=...) -> None: '''Wait for the latch with the given key to reach zero. If ``strict`` is ``True`` and the key doesn't exist, :exc:`KeyError` is raised.'''
    async def wait_all(self, timeout: float|None=...) -> None: '''Wait until the count of all latches reach zero.'''
    @property
...
class RLock(LockWithOwnerMixin[None]):
    ...
    def __init__(self, lock: AsyncContextManager[Any]|None=...): ...
