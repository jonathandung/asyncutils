...
class ConsoleBase(InteractiveConsole, ABC):
    ...
    memory_error_hooks: ClassVar[tuple[Callable[[Self], Any], ...]]
    '''Functions called when a :exc:`MemoryError` occurs, in that order, besides essential hardcoded logic.

    .. note:: Add hooks using the ``additional_memory_error_hooks`` class construction parameter.'''
    ...
    @final
    def memory_error(self) -> None: '''Pass ``additional_memory_error_hooks`` to the subclass constructor to change the behaviour when encountering a :exc:`MemoryError`, instead of touching this method.'''
    def write_special(self, msg: str) -> None: '''Write the banner and exit messages. Can have a different implementation than :meth:`~code.InteractiveInterpreter.write`.'''
