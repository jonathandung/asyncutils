...
class LineProtocol(Protocol, LoopMixinBase):
    ...
    def eof_received(self) -> None: '''Called when the other end signals it won't send any more data, for example by calling :meth:`~asyncio.WriteTransport.write_eof`, which closes the transport.'''
...
class SocketTransport(Transport):
    ...
    def close(self, e: Exception|None=...) -> None: '''Close the transport and flush the outgoing data buffer. No more data will be received. After all buffered data is flushed, the protocol's :meth:`~asyncio.BaseProtocol.connection_lost` method will be called with ``None`` as argument. The transport is not to be used once closed.'''
