# ruff: noqa: PLW0603,PLW2901 # ty: ignore[unresolved-attribute]
...
def f(a, b, f=S.audit, _=O.pipe, /):
    def double_ended_pipe(*, pipe_impl=_, x=partial(open, mode=a), y=partial(open, mode=b), f=f): r, W, R, w = *pipe_impl(), *pipe_impl(); f(f'asyncutils.iotools.double_ended_{'text' if a == 'r' else 'binary'}_pipe', r, w, R, W); return tuple(map(AsyncReadWriteCouple, map(x, (r, R)), map(y, (w, W))))
    return double_ended_pipe
z, s = lambda s=None, /, **d: {k: v for k, v in d.items() if v is not s}, '*, pipe_impl={}'
...
class File(A.LoopContextMixin): # noqa: PLR0904
    ...
    def read(self, offset=0, size=-1): return self._run(self._read, offset, size)
    def write(self, data, offset=0): return self._run(self._write, data, offset)
    async def readline(self, offset=0, size=None, include_newline=False): return (await self._run(self._readline, offset, size, include_newline))[0]
    async def readlines(self, hint=-1): return list(await self._run(self._readlines, hint))
    async def flush(self, offset=0, size=None, /): return await self._run(self._flush, offset, size)
    ...
    async def delete(self, offset, size):
        if size <= 0 or offset >= (s := await self._run(self.__m.size)): return
        if (t := offset+size) < s: await self.write(await self.read(t), offset)
        await self._run(self.resize, max(0, s-size))
    ...
    def __init_subclass__(cls, *, m, r):
        async def _run(f, /, *a, r=r): return await r(f, *a)
        cls.mgr, cls._run, cls.open_files = m, staticmethod(_run), {}
class MemoryMappedIOManager(A.LoopContextMixin):
    ...
    async def prefetch_files(self, *P, init_size=0, _=S.exc_info):
        ...
        try: yield await gather(*(c.__aenter__() for c in l)) # noqa: PLC2801
    ...
    async def find_in_files(self, pattern, paths, max_per_file=None, *, allow_overlapping=False):
        async def search(p, o):
            async with self.open(p) as f: return p, await (f.search if allow_overlapping else f.search_non_overlapping)(pattern, o, max_per_file)
        return {k: v for k, v in await gather(*starmap(search, paths.items())) if v}
    P.patch_method_signatures((__init__, 'executor=None'), (prefetch_files, '*paths, init_size=0'), (_open, 'init_size, path, mode, /'))
def __getattr__(n, /, _=S, a=frozenset(('ainput', 'stdcoup')), g=globals()):
