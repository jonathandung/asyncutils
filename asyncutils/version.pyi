...
class VersionInfo(str): # noqa: FURB189
    ...
    def __getitem__(self, idx: ValidSlice, /) -> tuple[int, ...]: # ty: ignore[invalid-method-override]
        ...
        * ``2``: :attr:`patch`
        ...
        Slicing works as if the version object is a tuple of these three items, which is also accessible as :attr:`parts`.
        '''
    ...
    def __index__(self) -> int: '''Identical to :meth:`__int__`.'''
    ...
    def _replace(self, major: int=..., minor: int=..., patch: int=...) -> Self: '''Satisfy the named tuple interface.'''
    ...
    def change_sep(self, sep: str) -> str: '''Return this version as a string with the specified separator instead of a dot between parts.'''
    def compatible(self, other: Self, /, maj_tol: int|None=..., min_tol: int|None=...) -> bool: '''Whether this version is compatible with the other, given the major and minor tolerances.'''
    @property
...
def normalize(o: object, /) -> tuple[int, int, int]:
    ...
    '''
def normalize_allow_unimplemented(o: object, /) -> tuple[int, int, int]|None: '''Like :func:`normalize`, but returns ``None`` if a normalizer is not found.'''
@overload
...
def autogenerate_normalizers() -> bool: '''Register normalizers for :class:`decimal.Decimal` and :class:`fractions.Fraction`. Return whether both normalizers were successfully registered, including re-registration of the same normalizer.'''
