...
@overload
async def locked_lock() -> Lock: '''Return an already acquired lock of type ``lcls`` if passed and :class:`asyncio.Lock` by default.'''
@overload
def done_evt[T: EventProtocol](*, evtcls: type[T]) -> T: ...
@overload
...
@overload
def done_fut(exc: ExceptionWrapper, /, *, futcls: type[FutProtocol[Any]]) -> IncompleteFut[Never]: ...
@overload
...
def get_future[T](aw: Awaitable[T], loop: AbstractEventLoop|None=...) -> Future[T]:
    ...
    | Critical exceptions are wrapped in :class:`~asyncutils.exceptions.Critical`.
...
@overload
def semaphore(bounded: Literal[True], workers: int=...) -> BoundedSemaphore: '''Return a (bounded) semaphore of value ``workers``, defaulting to :const:`~asyncutils.context.Context.SEMAPHORE_DEFAULT_VALUE`.'''
...
@overload
def dualcontextmanager(*, strict: Literal[True]) -> StrictDualContextFactory: ...
@overload
