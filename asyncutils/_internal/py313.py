# ty: ignore[unresolved-attribute]
import heapq as H
__all__ = 'Placeholder', 'heapify', 'heappop', 'heappush', 'heappushpop', 'heapreplace', 'partial'
heapify, heappop, heapreplace, Placeholder = H._heapify_max, H._heappop_max, H._heapreplace_max, object()
def heappush(heap, item, /, _=H._siftdown_max): heap.append(item); _(heap, 0, len(heap)-1)
def heappushpop(heap, item, /, _=H._siftup_max):
    if heap and item < (m := heap[0]): item, heap[0] = m, item; _(heap, 0)
    return item
def _get_merger(A, _=__import__('operator').itemgetter):
    if not A: return 0, None
    j = n = len(A); f = (O := []).append
    for i, a in enumerate(A):
        if a is Placeholder: f(j); j += 1
        else: f(i)
    j -= n; return j, _(*O) if j else None
class partial: # noqa: N801
    __slots__ = '__weakref__', '_merger', '_phcount', 'args', 'func', 'keywords'
    @__import__('reprlib').recursive_repr()
    def __repr__(self): (f := (A := [repr(self.func)]).extend)(map(repr, self.args)); f(f'{k}={v!r}' for k, v in self.keywords.items()); return f'asyncutils._internal.py313.partial({', '.join(A)})'
    def __new__(cls, f, /, *a, **k): return cls._new(f, a, k)
    @classmethod
    def _new(cls, f, a, k, g=_get_merger, r=__import__('itertools').repeat, /):
        if a and a[-1] is Placeholder: raise TypeError('trailing Placeholders are not allowed')
        if k is None: k = {}
        if any(v is Placeholder for v in k.values()): raise TypeError('Placeholder cannot be passed as a keyword argument')
        if isinstance(f, cls):
            P, e = f._phcount, (A := list(f.args)).extend
            if a:
                e(a)
                if P:
                    if (N := len(a)) < P: e(Placeholder for _ in r(None, P-N))
                    A = list(f._merger(A))
                    if N > P: A.extend(a[P:])
                C, M = g(A)
            else: C, M = P, f._merger
            k, f = f.keywords|k, f.func
        else: C, M = g(A := a)
        (s := object.__new__(cls)).func, s.args, s.keywords, s._phcount, s._merger = f, tuple(A), k, C, M; return s
    def __call__(self, /, *a, **k):
        if (c := self._phcount):
            try: A, a = self._merger(self.args+a), a[c:]
            except IndexError: raise TypeError(f"missing positional arguments in 'partial' call; expected at least {c}, got {len(a)}") from None
        else: A = self.args
        return self.func(*A, *a, **self.keywords, **k)
    def __get__(self, o, _=None): return self if o is None else type(self.__get__)(self, o)
    def __reduce__(self): return type(self)._new, (self.func, self.args, self.keywords or None)
del _get_merger, H
