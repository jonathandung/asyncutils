...
def patch_function_signatures(*to_patch: SigPatcherArg, follow_wrapped: bool=...) -> None:
    ...
    | If `follow_wrapped` is `True`, the original signature is taken from the first wrapped function with a :attr:`__text_signature__`
    | attribute or the innermost function instead of the wrapper, if applicable. Wrapped functions are found using :attr:`__func__`
    | and :attr:`__wrapped__`, and it is assumed in the implementation that every level only has one of those, otherwise unpredictable
    | behaviour may arise.
...
xsig: Final[str]
'''The signature of :meth:`__exit__` and :meth:`__aexit__` as a signature-patcher-compatible string. You would usually only pass this to :func:`patch_method_signatures`.'''