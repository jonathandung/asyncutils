...
if c := (E := __import__('os').environ).get(k := 'AUTILSCFGPATH', '').strip('"\' \t\r\n\v\f'): # pragma: no cover
    ...
    a('asyncutils/recurse_configs', A := __import__('pathlib').Path.cwd()); f, s, t = None, 'asyncutils/try_config', 'pyproject.toml'
    for i, A in enumerate(__import__('itertools').chain((A,), A.parents)):
        if (A := A/t).is_file():
            a(s, i)
            with A.open('rb') as f:
                try: d = __import__('tomllib').load(f)['tool']['asyncutils']; a('asyncutils/read_config', c := str(A)); break
                except KeyError: ...
    else: d = {}
    del A, f, s, t, i
if v := d.pop('context', None):
    for v in v.values():
        if (a := type(v)) is not dict: m('key "context" in %s should be an object mapping submodule names to objects, not %r', H.fullname(a))
        for K, V in v.items():
            if type(V) is dict:
                for k, v in V.items(): C[f'{K}_{k}'.upper()] = v # noqa: PLW2901
            elif (K := K.upper()) in C: C[K] = V
N.update(d)
del E, k, H, M, m, K, V, a, v, d