from .helpers import fullname, subscriptable as s
...
if p := (E := __import__('os').environ).get(k := 'AUTILSCFGPATH', '').strip('"\''):
    ...
    with open(p.strip()) as f:
        if (t := type(f := __import__('json').load(f))) is not dict: raise TypeError(f'incorrent json format for asyncutils configuration at {p}; top-level structure should be an object, not {fullname(t)!r}; see format.json5')
