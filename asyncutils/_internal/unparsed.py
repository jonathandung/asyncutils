...
if c := (E := __import__('os').environ).get(k := 'AUTILSCFGPATH', '').strip('"\' \t\r\n\v\f'):
    ...
    for i, A in enumerate(__import__('itertools').chain((A,), A.parents)): # noqa: B020
        if (A := A/t).is_file():
            a(s, i)
            with A.open('rb') as f:
    ...
    del A, f, s, i
for S in d.pop('context', {}).values(): # noqa: F402
