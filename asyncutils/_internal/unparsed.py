...
if c := (E := __import__('os').environ).get(k := 'AUTILSCFGPATH', '').strip('"\' \t\r\n\v\f'):
    from sys import audit as a; a('asyncutils/read_config', c); K = V = None
    if isinstance(v := (P := (d := l(c)).pop)('next_config', c), str): a('asyncutils/set_next_config', c, v); E[k] = v
    elif v is None: a('asyncutils/discontinue_config', c); del E[k]
    else: raise TypeError(f'key "next_config" in {c} should point to a string or null, not {v!r}; see format.json5')
    for E in () if (e := P('context', None)) is None else e.values():
        if (a := type(E)) is not dict: raise TypeError(f'incorrent json format for asyncutils configuration at {c}; "context" should be an object mapping to objects, not {g(a)!r}; see format.json5')
        for K, V in E.items():
            if type(V) is dict:
