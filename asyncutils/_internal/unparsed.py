...
def l(p, e=None, /):
    with open(p) as F:
        match L := D[e or p.rpartition('.')[-1]]: # pragma: no cover
            ...
            case 'configparser': raise RuntimeError('cannot parse ini-like format due to type ambiguities; you should rewrite the configuration in TOML, which is the most similar to ini')
            case _:
