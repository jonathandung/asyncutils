...
def subscriptable[T](cls: type[T], /) -> Subscriptable[T]: '''Add a :meth:`~object.__class_getitem__` method to the given class that returns a :class:`~types.GenericAlias`, raising :exc:`TypeError` if it is already present.'''
def check(candidate: object, against: object, /) -> bool: '''Check if two objects are equal according to :meth:`~object.__eq__` of ``against`` only, such that it cannot falsely claim equality.'''
...
@overload
def coerce_callable[T: Not[Callable[..., Any]]](o: T, /) -> type[T]: '''Return the argument itself if callable and its type otherwise.'''
...
class LoopMixinBase:
    '''Much like :class:`!asyncio.mixins._LoopBoundMixin`.'''
    @property
