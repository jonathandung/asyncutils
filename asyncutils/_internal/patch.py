...
def _(t, d, a='__text_signature__', u='<unrepresentable>', c=('__wrapped__', '__func__')):
    def p(*I, follow_wrapped=True):
        m, f = {}, True
        for k, v in I:
            if follow_wrapped:
                while not (hasattr(k, a) or (i := id(k)) in m): m[i], k, f = k, getattr(k, c[f], k), not f # ruff: ignore[redefined-loop-name]
...
exit_sig = 'exc_typ, exc_val, exc_tb, /'
del _
