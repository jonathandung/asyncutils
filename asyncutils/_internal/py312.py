...
if __import__('sys').version_info < (3, 13):
    ...
    class Queue(h.LoopMixinBase):
        ...
        def __str__(self, *e, _=h.fullname):
            f = (p := [_(self), *e, f'maxsize={self.maxsize!r}']).append
        ...
        # ruff: disable[suppressible-exception]
        ...
        def put_nowait(self, item):
            if self._is_shutdown: raise QueueShutDown
            if self.full(): raise __import__('asyncio').QueueFull
        ...
        # ruff: enable[suppressible-exception]
        def get_nowait(self):
            if self.empty(): raise QueueShutDown if self._is_shutdown else __import__('asyncio').QueueEmpty
    ...
    # ruff: disable[unused-method-argument]
    ...
    # ruff: enable[unused-method-argument]
    del H, h
else: from asyncio.queues import * # ruff: ignore[undefined-local-with-import-star]
