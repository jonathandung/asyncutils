...
class PartialInterface(metaclass=PartialInterfaceMeta):
    ...
    that may actually exist on the object/class. This is a simplistic fix that assumes those attributes always exist and makes no attempt to infer their types.'''
...
class MemoryMappedFile(LoopContextMixin):
    '''The type of async memory-mapped files as opened by `io.MemoryMappedIOManager`. Calls itself `asyncutils.io.File`.'''
