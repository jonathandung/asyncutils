...
class CacheWithBackgroundRefresh(A.LoopContextMixin):
    ...
    def __init__(self, ttl=None, refresh=None, *, processor=None, default_loader=None, timer=monotonic, _=defaultdict):
        ...
        audit(fullname(self), ttl, refresh); super().__init__(); self._cache, self._lock, self._loaders, self._task, self._event, self._timer = {}, I.Lock(), _(lambda _=default_loader, /: _), None, I.Event(), timer; self.configure(ttl, refresh, processor)
...
class AsyncLRUCache:
    ...
    def __init__(self, maxsize=None, ttl=None, *, typed=False, timer=monotonic):
        if maxsize is None: maxsize = A.getcontext().ASYNC_LRU_CACHE_DEFAULT_MAX_SIZE
        audit(fullname(self), maxsize, ttl)
        self._ttl, self._factory, self._timestamps, self._caches, self._loopctx, self._timer = ttl, lru_cache(maxsize, typed), {}, {}, A.event_loop.from_flags(0x200), timer
    def __call__(self, f, /, _=lambda f, a, k, s=0x3d, t=0x7a: id(f)<<t|hash(a)<<s|hash(frozenset(k.items()))):
        self._caches[f] = c = self._factory(f)
        if not hasattr(self, '_make_key'): self._make_key = A.to_async(_, self.loop)
        async def wrapper(*a, **k):
            ...
            if I.iscoroutine(r := c(*a, **k)): r = await r
            if t: S[K] = T()
            return r
