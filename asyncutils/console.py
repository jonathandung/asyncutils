...
class ConsoleBase(__import__('_pyrepl.console', fromlist=_f).InteractiveColoredConsole):
    ...
    def interact(self, banner=None, *, ps1='>>> ', _f=_f, _s=_s, _q=C.silent, _o=type('', (), {'write': lambda *_: None, 'flush': lambda _, /: None})(), _g=getenv):
        ...
        try:
            if p := _g('PYTHONSTARTUP'):
                with __import__('tokenize').open(p) as f:
                    if _q: sys.stdout, _o = _o, sys.stdout
                    sys.audit('cpython.run_startup', p); exec(compile(f.read(), p, 'exec'), self.locals)
                    if _q: sys.stdout = _o
            if (p := getattr(sys, 'ps1', None)) is None: p, x = ps1, True
        ...
        finally: self._loop.stop(_s); sys.ps1 = ps1 if x else getattr(sys, 'ps1', ps1) if p is None else p
    def _interact_hook(self, ps1, kcolor, reset, fcolor): n, sys.ps1 = self.NAME, ps1; self.write_special(f'{ps1}{kcolor}import{reset} {n}\n{ps1}{kcolor}from{reset} {n} {kcolor}import{reset} *\n')
    ...
    def run(self, *, exitmsg='Thank you for using %s!\nExiting REPL...\n', threadname='<%s REPL thread>', max_memerrs=None, always_run_interactive=bool(sys.flags.inspect), always_install_completer=False, suppress_asyncio_warnings=False, suppress_unawaited_coroutine_warnings=False):
        ...
        if always_run_interactive or sys.stdin.isatty():
            ...
            if callable(h := getattr(sys, i := '__interactivehook__', None)):
                sys.audit('cpython.run_interactivehook', h)
                try: h()
...
class AsyncUtilsConsole(ConsoleBase, version=V, description='asyncutils is a multi-purpose and efficient asynchronous utilties library.\nYou can use await statements directly instead of asyncio.run for quick testing.\nAll the submodules of asyncutils are also loaded into the namespace.\nDo not use functions such as sync_await in this REPL, they are bound to cause deadlocks.', native_handler=lambda d, /, v=V, _=_f: d.update(m := __import__('asyncutils._internal.initialize', fromlist=_).s) or setattr(f := lambda m=m, /: m.update({k: v if (g := getattr(v, 'load', None)) is None else g() for k, v in m.items()}), '__qualname__', l := 'load_all') or setattr(f, '__name__', l) or d.update(__version__=v, load_all=f), default_local_exit=True, disallow_subclass_msg='cannot subclass %s; subclass {}console.ConsoleBase instead'.format(pkgpref)):
    ...
    def posthook(self, _m='WARNING: user tampered with asyncutils module state\n', _n=pkgpref+'console'):
        if _unset_() is not self: sys.stderr.write(_m); del sys.modules[_n]
        super().posthook()
    def showtraceback(self, _skip_frames=3, _suf=('asyncutils\\console.py', 'asyncutils/console.py'), _fln=31, _mn=sys.intern('__callback')):
        t, v, b = sys.exc_info()
        try:
...
del _f, _s, pkgpref, getenv, C, debug, V