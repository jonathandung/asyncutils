...
from .mixins import ExecutorRequiredAsyncContextMixin, LoopBoundMixin, LoopContextMixin
from _collections_abc import Callable, Generator, Iterable, Mapping
from asyncio.futures import Future
from collections import deque
from types import TracebackType
from typing import Any, Self, overload
__all__ = 'AdvancedPool', 'CallbackAccumulator', 'ConnectionPool'
class AdvancedPool(LoopContextMixin):
    ...
    @overload
    def submit_nowait[T](self, f: Callable[..., T], *a: Any, _priority_: int, **k: Any) -> Future[T]: '''Schedule a sync function to be executed in a worker thread, raising :exc:`asyncio.QueueFull` if there is not enough space in the internal queue from which workers fetch tasks, and get an async future to access its result.'''
    @overload
    async def submit[T, **P](self, f: Callable[P, T], *a: P.args, **k: P.kwargs) -> Future[T]: ...
    @overload
    async def submit[T](self, f: Callable[..., T], *a: Any, _priority_: int, **k: Any) -> Future[T]: '''Schedule a sync function to be executed in a worker thread, waiting asynchronously until there is enough space in the internal queue from which workers fetch tasks if necessary, and get an async future to access its result.'''
    @overload
    async def complete[T, **P](self, f: Callable[P, T], *a: P.args, **k: P.kwargs) -> T: ...
    @overload
    async def complete[T](self, f: Callable[..., T], *a: Any, _priority_: int, **k: Any) -> T: '''Wait for a sync function to complete its execution by the pool asynchronously and get its result.'''
...
class ConnectionPool[T, **P](LoopBoundMixin):
    ...
    @property
    def currsize(self) -> int: '''Return the current number of connections managed by the pool, including those in use and those available.'''
    @property
    def available(self) -> int: '''Return the number of connections currently available for acquisition.'''
    @property
    def in_use(self) -> int: '''Return the number of connections currently in use.'''
    @property
    def maxsize(self) -> int: '''Return the maximum number of connections that can be created by the pool.'''
    @property
    def minsize(self) -> int: '''Return the minimum number of connections that will be maintained by the pool.'''
    @property
    def maxlife(self) -> float: '''Return the maximum lifetime of a connection in seconds.'''
class CallbackAccumulator[T, **P](deque[T], ExecutorRequiredAsyncContextMixin[CallbackAccumulator[T, P]]):
    ...
    def __init__(self, name: str, *, maxlen: int|None=..., default: object=..., call_once: bool=..., default_getter: Callable[[], tuple[Iterable[object], Mapping[str, object]]]=...):
        ...
        * `name` is used for debugging purposes only.
    ...
    def offer_last(self, o: object, /) -> bool: ...
    @property
