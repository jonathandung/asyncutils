...
class AdvancedPool(LoopContextMixin):
    ...
    def __init__(self, max_workers: int=..., min_workers: int=..., qsize: int=..., scaling: bool=..., kill_at_exit: bool=...):
        ...
        * ``kill_at_exit`` determines whether the shut down when the context manager exits should be immediate. Default ``False``.
        '''
    def raise_for_shutdown(self) -> None: '''Raise :exc:`~asyncutils.exceptions.PoolShutDown` if the pool is shutting down or has been shut down.'''
    @overload
    ...
    @property
    def empty(self) -> bool: '''Whether the internal queue for pending tasks is empty.'''
    @property
    def qsize(self) -> int: '''The current number of pending tasks in the internal queue.'''
    @property
...
class ConnectionPool[T, **P](LoopMixinBase):
    ...
    def __init__(self, factory: Callable[P, T], maxsize: int=..., minsize: int=..., maxlife: float=..., healthchecker: Callable[[T], bool]|None=..., cleaner: Callable[[T], None]|None=...):
        ...
        * ``maxlife`` sets the maximum lifetime of a connection in seconds, after which it will be recycled. Defaults to :const:`~asyncutils.context.Context.CONNECTION_POOL_DEFAULT_MAX_LIFE`.
    ...
    @overload
    async def create_connection(self, *a: object, _executor_: Executor|None, **k: object) -> T: '''Call the connection factory with the given parameters and return a new connection object.'''
    ...
    @property
    def cursize(self) -> int: '''The current number of connections managed by the pool, including those in use and those available.'''
    @property
