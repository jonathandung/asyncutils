...
class AsyncReadWriteCouple(LoopContextMixin):
    ...
    def __init__(self, r, w, /): super().__init__(); self.loop.set_task_factory(eager_task_factory); self.reader, self.writer, self._executor = r, w, Executor()
    ...
    def fileno(self): raise TypeError('cannot determine fileno of read-write couple')
    def isatty(self): return self.reader.isatty() and self.writer.isatty()
    ...
    def seek(self, offset, whence=0, /): raise TypeError('cannot use seek on read-write couple')
    def tell(self): raise TypeError('cannot use tell on read-write couple')
...
class file(LoopContextMixin):
    ...
    def __del__(self): self.loop.run_until_complete(self.close()); del self.loop
    @property
...
class MemoryMappedIOManager(LoopContextMixin):
    ...
    def __del__(self): sync_await(self.__cleanup__(), loop=(l := self.loop)); l.stop(); l.close(); del self.loop
