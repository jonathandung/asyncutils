...
from _collections_abc import AsyncIterable, AsyncGenerator, Awaitable, Callable, Hashable, Iterable, Mapping, MutableSequence, Reversible
...
async def aunzip(ait: SupportsIteration[tuple[Any, ...]], *, put_batch: int=..., fillvalue: object=..., maxqsize: int=...) -> tuple[AUnzipConsumer[Any], ...]:
    ...
    | :const:`context.AUNZIP_DEFAULT_PUT_BATCH`) and caching other items in queues of capacity `maxqsize` (default :const:`context.AUNZIP_DEFAULT_MAX_QSIZE`).
    | This function may thus require significant auxiliary space.'''
...
def asplitat[T](it: SupportsIteration[T], pred: Callable[[T], object], maxsplit: int=..., keep_sep: bool=...) -> AsyncGenerator[list[T]]: '''Split an async iterator at each item satisfying `pred`, with `keep_sep` dictating '''
...
@overload
def amultimapif[*Ts, R](f: Callable[[*Ts], R], pred: Callable[[tuple[*Ts]], object], /, *its: SupportsIteration[Any], await_: Literal[False]=...) -> AsyncGenerator[R]: ...
@overload
def amultimapif[*Ts, R](f: Callable[[*Ts], Awaitable[R]], pred: Callable[[tuple[*Ts]], object], /, *its: SupportsIteration[Any], await_: Literal[True]) -> AsyncGenerator[R]: '''Composition of :func:`astarmap`, :func:`afilter` and :func:`azip`.'''
@overload
...
def adropwhile[T](pred: Callable[[T], object], it: SupportsIteration[T], *, skip_first: bool=...) -> AsyncGenerator[T]:
    '''| Async version of :class:`itertools.dropwhile` that is not a class.
    | If `skip_first` is `True`, drop also the first item that fails the predicate.'''
def afilterfalse[T](f: Callable[[T], object]|None, it: SupportsIteration[T]) -> AsyncGenerator[T]: '''Async version of :class:`itertools.filterfalse` that is not a class.'''
...
def atakewhile[T](pred: Callable[[T], object]|None, it: SupportsIteration[T]) -> AsyncGenerator[T]: '''Async version of :class:`itertools.takewhile` that is not a class.'''
...
def ac3merge[T](seqs: SupportsIteration[MutableSequence[T]]) -> AsyncGenerator[T]: '''Async version of :func:`functools._c3_merge` that doesn't assume the input is an synchronous iterable of mutable sequences of classes.'''
...
@overload
def aziplongest[T, R](i1: SupportsIteration[T], i2: SupportsIteration[R], /, *, fillvalue: object=...) -> AsyncGenerator[tuple[T, R], None]: ...
@overload
def aziplongest[T, R, V](i1: SupportsIteration[T], i2: SupportsIteration[R], i3: SupportsIteration[V], /, *, fillvalue: object=...) -> AsyncGenerator[tuple[T, R, V], None]: ...
@overload
def aziplongest[T, R, V, U](i1: SupportsIteration[T], i2: SupportsIteration[R], i3: SupportsIteration[V], i4: SupportsIteration[U], /, *, fillvalue: object=...) -> AsyncGenerator[tuple[T, R, V, U], None]: ...
@overload
def aziplongest[T, R, V, U, S](i1: SupportsIteration[T], i2: SupportsIteration[R], i3: SupportsIteration[V], i4: SupportsIteration[U], i5: SupportsIteration[S], /, *, fillvalue: object=...) -> AsyncGenerator[tuple[T, R, V, U, S], None]: ...
@overload
...
@overload
def amultifilterfalse(pred: Callable[[tuple[Any, ...]], object], i1: SupportsIteration[Any], i2: SupportsIteration[Any], i3: SupportsIteration[Any], i4: SupportsIteration[Any], i5: SupportsIteration[Any], /, *its: SupportsIteration[Any], strict: bool=...) -> AsyncGenerator[tuple[Any, ...]]: '''Composition of :func:`afilterfalse` and :func:`azip`.'''
@overload
...
@overload
def amultistarfilter(pred: Callable[[*tuple[Any, ...]], object], i1: SupportsIteration[Any], i2: SupportsIteration[Any], i3: SupportsIteration[Any], i4: SupportsIteration[Any], i5: SupportsIteration[Any], /, *its: SupportsIteration[Any], strict: bool=...) -> AsyncGenerator[tuple[Any, ...]]: '''Composition of :func:`astarfilter` and :func:`azip`.'''
@overload
...
@overload
def amultistarfilterfalse(pred: Callable[[*tuple[Any, ...]], object], i1: SupportsIteration[Any], i2: SupportsIteration[Any], i3: SupportsIteration[Any], i4: SupportsIteration[Any], i5: SupportsIteration[Any], /, *its: SupportsIteration[Any], strict: bool=...) -> AsyncGenerator[tuple[Any, ...]]: '''Composition of :func:`astarfilterfalse` and :func:`azip`.'''
...
def acombinations[T](it: SupportsIteration[T], r: int) -> AsyncGenerator[tuple[T, ...], None]: '''Async version of :func:`itertools.combinations` that is not a class.'''
def acombinations_with_replacement[T](it: SupportsIteration[T], r: int) -> AsyncGenerator[tuple[T, ...], None]: '''Async version of :func:`itertools.combinations_with_replacement` that is not a class.'''
def apermutations[T](it: SupportsIteration[T], r: int|None=...) -> AsyncGenerator[tuple[T, ...], None]: '''Async version of :func:`itertools.permutations` that is not a class.'''
def apowerset[T](it: SupportsIteration[T]) -> AsyncGenerator[tuple[T, ...], None]: '''Yield all the subsets of the (async) iterable `it` (by index!) as tuples, starting with the empty tuple.'''
...
def agrouper[T](it: SupportsIteration[T], n: int, fillvalue: T|RaiseType=...) -> AsyncGenerator[tuple[T|None, ...], None]:
    '''| Collect items of the (async) iterable `it` into tuples of length `n` each.
    | If `fillvalue` is :const:`constants.RAISE`, raise :exc:`ValueError` if the last group is not of length `n`.
    | Otherwise, pad the last group with `fillvalue` to length `n` if needed. No padding is done if not passed.'''
def aroundrobin[T](*its: SupportsIteration[T]) -> AsyncGenerator[T]: '''Yield items from the given (async) iterables in round-robin order, skipping exhausted iterables. Prefer for less iterables.'''
def aroundrobin2[T](*its: SupportsIteration[T]) -> AsyncGenerator[T]: '''Yield items from the given (async) iterables in round-robin order, skipping exhausted iterables. Prefer for more iterables.'''
...
async def ailen(it: SupportsIteration[Any]) -> int: '''Return the length of the (async) iterable `it`, consuming it entirely.'''
...
def with_aiter[T](actxmgr: AsyncContextManager[SupportsIteration[T]]) -> AsyncGenerator[T]: '''Yield items from the iterable the entry result of `actxmgr` gives until exhausted, then close `actxmgr`.'''
@overload
...
def adistinct_permutations[T](it: SupportsIteration[T], r: int|None=...) -> AsyncGenerator[tuple[T, ...], None]: '''Successive distinct permutations of the elements in `it` of size `r`, or all sizes if not passed.'''
...
def ainterleave_evenly[T](its: SupportsIteration[SupportsIteration[T]], lengths: SupportsIteration[int]|None=...) -> AsyncGenerator[T]: '''Interleave items of the iterables evenly according to the lengths if passed, and determined by calling the :meth:`__len__` method on the iterables if present otherwise.'''
...
def aflatten_tensor(tensor: SupportsIteration[Any], base_typ: tuple[type, ...]|type=...) -> AsyncGenerator[Any]: ''':func:`acollapse`, but using a different, more memory-efficient strategy that does not support the `levels` parameter.'''
...
def asubstrings[T](it: SupportsIteration[T]) -> AsyncGenerator[tuple[T, ...], None]: '''Yield all the contiguous subsequences of the (async) iterable `it` as tuples, in increasing order of length.'''
@overload
def asubstr_indices[S: (str, bytes, bytearray)](seq: S, reverse: bool=...) -> AsyncGenerator[tuple[S, int, int], None]: ...
@overload
def asubstr_indices[T](seq: SupportsSlicing[T], reverse: bool=...) -> AsyncGenerator[tuple[Iterable[T], int, int], None]: '''Yield tuples of the form `(substr, start, end)`, where `substr` is a contiguous subsequence of `seq` starting at index `start` and ending at index `end-1` (such that its length is `end-start`).'''
def iter_task[I: SupportsIteration[Any]](it: I, summaryf: Callable[[I], Awaitable[Any]]=...) -> Task[float]: '''Return a task that calls `summaryf` on the passed (async) iterable and returns the time taken to run it. By default, `summaryf` consumes `it` fully.'''
