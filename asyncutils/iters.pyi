...
def areversed[T](it: SupportsIteration[T]|Reversible[T], /) -> AsyncGenerator[T]: '''Reverse an (async) iterable. Call the :meth:`__reversed__` method if present, falling back to consuming all the items and yielding them in reverse order.'''
...
@overload
def apowersetofsets[T: Hashable](it: SupportsIteration[T], *, frozen: Literal[False]) -> AsyncGenerator[set[T]]: '''Yield all the subsets of the items in the (async) iterable after consuming it at once, as :class:`frozenset`'s if `frozen` is `True` (the default) and `set`s otherwise.'''
