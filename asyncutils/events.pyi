...
class SingleWaiterEventWithValue[T](EventMixin[T]):
    ...
    def get(self, default: T=...) -> T: '''Get the result of the event immediately if set, otherwise returning `default` if passed or throw `RuntimeError`.'''
class EventWithValue[T](EventMixin[T]):
    '''An event that can store a value and maintains a history of past values.'''
    def __init__(self, *, maxhist: int|None=...) -> None: '''A maximum of `maxhist` entries, which defaults to `context.EVENT_WITH_VALUE_DEFAULT_MAX_HIST`, of past results are stored.'''
    @overload
    ...
    async def wait_for_transition(self, old: T, new: T, timeout: float|None=..., *, force_transition: bool=...) -> bool:
        ...
        `timeout` defaults to `context.EVENT_WITH_VALUE_DEFAULT_TIMEOUT`, and on its expiry, if `force_transition` is `True`, cause the transition to happen manually.
