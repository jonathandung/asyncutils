...
class EventWithValue[T](EventMixin):
    def __init__(self, *, maxhist: int|None=...): '''Initialize an EventWithValue, storing a maximum of `maxhist` entries of past results, which defaults to `constants.EVENT_WITH_VALUE_DEFAULT_MAXHIST`.'''
    @overload
    ...
    def recent_history(self, duration: float|None=...) -> Generator[tuple[float, T], None, None]: '''Yield recent history entries in order; what qualifies as recent depends on `duration`, defaulting to `constants.EVENT_WITH_VALUE_DEFAULT_RECENT`.'''
    async def wait_for_transition(self, old: T, new: T, timeout: float|None=..., *, force_transition: bool=...) -> bool:
        '''Wait until the value is set to `old`, and then `new`. The timeout for which defaults to `constants.EVENT_WITH_VALUE_DEFAULT_TIMEOUT`.
