...
class BoundedBatchProcessor[T, R]:
    ...
    def __init__(self, processor: Callable[[list[T]], Awaitable[R]], batch: int=..., max_concurrent: int=...) -> None: '''``batch`` defaults to :data:`context.BOUNDED_BATCH_PROCESSOR_DEFAULT_BATCH_SIZE` and ``max_concurrent`` :data:`context.BOUNDED_BATCH_PROCESSOR_DEFAULT_MAX_CONCURRENT`.'''
...
class BatchProcessor[T](LoopContextMixin):
    ...
    def __init__(self, processor: Callable[[list[T]], Awaitable[None]], *, maxsize: int=..., maxtime: float=..., timer: Timer=...) -> None: '''``maxsize`` defaults to :data:`context.BATCH_PROCESSOR_DEFAULT_MAX_SIZE`, ``maxtime`` to :data:`context.BATCH_PROCESSOR_DEFAULT_MAX_TIME`, and ``timer`` :func:`time.monotonic`.'''
...
class Bulkhead(LoopContextMixin):
    ...
    def __init__(self, max_concurrent: int, *, max_queue: int=..., max_rej: int=..., exc: Exceptable=..., processor: Callable[[BaseException], Awaitable[None]]=...) -> None:
        ...
        | ``max_queue``: maximum number of pending executions allowed in the queue; if the queue is full, new executions will be rejected until there is space in the queue. Non-positive value means no limit (there is currently no way to get a zero-capacity queue). Default :data:`context.BULKHEAD_DEFAULT_MAX_QUEUE`.
        | ``max_rej``: maximum number of rejections allowed before the bulkhead shuts down and rejects all new executions. Negative value means no limit. Default :data:`context.BULKHEAD_DEFAULT_MAX_REJ`.
    ...
    @property
    def available_qslots(self) -> float: '''The number of available slots in the task queue. Gives an integer, unless the queue is unbounded, in which case :const:`math.inf` is returned.'''
    @property
