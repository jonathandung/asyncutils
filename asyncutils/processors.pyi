...
class BoundedBatchProcessor[T, R]:
    ...
    def __init__(self, processor: Callable[[list[T]], Awaitable[R]], batch: int=..., max_concurrent: int=...): '''``batch`` defaults to :const:`~asyncutils.context.Context.BOUNDED_BATCH_PROCESSOR_DEFAULT_BATCH_SIZE` and ``max_concurrent`` :const:`~asyncutils.context.Context.BOUNDED_BATCH_PROCESSOR_DEFAULT_MAX_CONCURRENT`.'''
...
class BatchProcessor[T](LoopMixinBase):
    '''
    ...
    '''
    def __init__(self, processor: Callable[[list[T]], Awaitable[None]], *, maxsize: int=..., maxtime: float=..., timer: Timer=...): '''``maxsize`` defaults to :const:`~asyncutils.context.Context.BATCH_PROCESSOR_DEFAULT_MAX_SIZE`, ``maxtime`` to :const:`~asyncutils.context.Context.BATCH_PROCESSOR_DEFAULT_MAX_TIME`, and ``timer`` :func:`time.monotonic`.'''
...
class Bulkhead(LoopMixinBase):
    ...
    .. caution:: Use instances of this class as async context managers to ensure proper cleanup.
    '''
    def __init__(self, max_concurrent: int, *, max_queue: int=..., max_rej: int=..., exc: CanExcept=..., processor: Callable[[BaseException], Awaitable[None]]=...):
        '''
