...
class TokenBucket:
    ...
    async def consume(self, tokens=None):
        if tokens is None: tokens = float(constants.TOKEN_BUCKET_DEFAULT_CONSUME_TOKENS)
...
class LeakyBucket(AsyncContextMixin, EventualLoopMixin):
    def __init__(self, capacity, leak, min_factor=None, max_factor=None, external_factor_settable=None, timer=monotonic): C = constants.getcontext(); self._leak, self._last, self._lock, self._drainer, self._factor, self._min_factor, self._max_factor, self._external_factor_settable, self._timer = leak, timer(), Lock(), None, 1.0, C.LEAKY_BUCKET_DEFAULT_MINFACTOR if min_factor is None else min_factor, C.LEAKY_BUCKET_DEFAULT_MAXFACTOR if max_factor is None else max_factor, C.LEAKY_BUCKET_DEFAULT_EXT_CAN_SET_FACTOR if external_factor_settable is None else external_factor_settable, timer; self._capacity = self._tokens = capacity
    ...
    async def wait_for_tokens(self, amount=1):
        w, m = 0.0, constants.LEAKY_BUCKET_WAIT_FOR_TOKENS_TICK
        while True:
            async with self._lock:
