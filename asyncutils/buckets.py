...
class LeakyBucket(AsyncContextMixin, LoopBoundMixin):
    __slots__ = '_capacity', '_drainer', '_external_factor_settable', '_factor', '_last', '_leak', '_max_factor', '_min_factor', '_timer', '_tokens'
    def __init__(self, capacity, leak, min_factor=None, max_factor=None, external_factor_settable=None, timer=monotonic): audit(fullname(self), capacity, leak); C = getcontext(); self._leak, self._last, self._drainer, self._factor, self._min_factor, self._max_factor, self._external_factor_settable, self._timer = leak, timer(), None, 1.0, C.LEAKY_BUCKET_DEFAULT_MIN_FACTOR if min_factor is None else min_factor, C.LEAKY_BUCKET_DEFAULT_MAX_FACTOR if max_factor is None else max_factor, C.LEAKY_BUCKET_DEFAULT_EXT_CAN_SET_FACTOR if external_factor_settable is None else external_factor_settable, timer; self._capacity = self._tokens = capacity
    ...
    async def _drain(self):
        while True: self._set_tokens(); self._adjust(); await sleep(min(1/(self._leak*self._factor), 1))
