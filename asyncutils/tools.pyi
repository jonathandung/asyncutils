...
def loadf(path: Openable, ext: str=..., /) -> dict[str, Any]: '''Load the file at ``path``, with the specified file extension if undeducible from the file name, into a dictionary using the correct library.'''
...
def argv_to_json(argv: Sequence[str], path: Openable, /, *, dump: DumpType=...) -> None:
    ...
    '''
def argstr_to_json(argstr: str, path: Openable, /, *, dump: DumpType=..., split: Callable[[str], Sequence[str]]=...) -> None: '''Parse the shell-escaped string representing the command-line arguments for this module and writes it into a .json path.'''
