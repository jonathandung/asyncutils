import asyncio, sys, os
from signal import Signals
from pytest import fixture, mark, raises, warns
from asyncutils import WarningToError
from asyncutils.signals import wait_for_signal
from tests.conftest import mk
async def processor(sig): return sig.value
class Log(BaseException): ...
def raise_(msg, *a, exc_info=False): raise Log(msg%a)
def ignore(*_): ...
@fixture(scope='module')
def wait_partial(): return __import__('_functools').partial(wait_for_signal, processor, logger=type(sys.implementation)(warning=raise_, error=raise_, exception=raise_, info=ignore, debug=ignore))
@mk
@mark.skipif(win := sys.platform == 'win32', reason='difficult to test signal handling on Windows')
async def test_signal_log(wait_partial):
    with raises(Log, match=r'signals\.wait_for_signal: invalid signal None'): await wait_partial(None)
    with raises(Log, match=r'signals\.wait_for_signal: timed out'): await wait_partial(4, timeout=0.01)
    with raises(Log, match=r'signals\.wait_for_signal: timed out' if sys.platform == 'darwin' else r'signals\.wait_for_signal: (insufficient permissions|error registering signal handler) for signal SIGSTOP'): await wait_partial(19, timeout=0.01)
@mk
async def test_signal_raise(wait_partial):
    with raises(TimeoutError), warns(RuntimeWarning, match=r'signals\.wait_for_signal has limited functionality on Windows') if win else WarningToError(): await wait_partial(Signals.SIGFPE, timeout=0.01, raise_on_timeout=True)
